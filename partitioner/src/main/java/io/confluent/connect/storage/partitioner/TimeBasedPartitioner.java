...
public class TimeBasedPartitioner<T> extends DefaultPartitioner<T> {
  ...
  public void configure(Map<String, Object> config) {
    long partitionDurationMsValue = (long) config.get(PartitionerConfig.PARTITION_DURATION_MS_CONFIG);
    if (partitionDurationMsValue < 0) {
      throw new ConfigException(PartitionerConfig.PARTITION_DURATION_MS_CONFIG,
                                partitionDurationMsValue, "Partition duration needs to be a positive.");
    }
    ...
    init(partitionDurationMsValue, pathFormat, locale, timeZone, config);
  }
  ...
  public TimestampExtractor newTimestampExtractor(String extractorClassName) {
    try {
      switch (extractorClassName) {
        ...
        case "Record":
        case "RecordField":
          extractorClassName = "io.confluent.connect.storage.partitioner.TimeBasedPartitioner$" + extractorClassName
                                   + "TimestampExtractor";
          break;
        default:
      }
    ...
    public Long extract(ConnectRecord<?> record) {
      ...
      if (value instanceof Struct) {
        ...
        switch (type) {
          ...
          default:
            log.error("Type {} is not supported as a user-defined record timestamp field.", type.getName());
